...
func buildSystemPrompt(skillToolName string, useChinese bool) (string, error) {
	return renderSystemPrompt(skillToolName, useChinese)
}
...
func (s *skillTool) Info(ctx context.Context) (*schema.ToolInfo, error) {
	...
	if s.customToolDesc != nil {
		...
		descBase, descErr := renderToolDescriptionBase(s.useChinese)
		if descErr != nil {
			return nil, fmt.Errorf("failed to render tool description base: %w", descErr)
		}
...
func defaultToolParams(useChinese bool) map[string]*schema.ParameterInfo {
	return map[string]*schema.ParameterInfo{
		"skill": {
			Type:     schema.String,
			Desc:     renderSkillParamDesc(useChinese),
			Required: true,
		},
	}
}
...
func (s *skillTool) buildParamsOneOf(ctx context.Context) (*schema.ParamsOneOf, error) {
	defaults := defaultToolParams(s.useChinese)
...
func (s *skillTool) defaultSkillContent(skill Skill) string {
	launch, _ := renderResultLaunch(skill.Name, s.useChinese)
	content, _ := renderResultContent(skill.BaseDirectory, skill.Content, s.useChinese)
...
func (s *skillTool) runAgentMode(ctx context.Context, skill Skill, forkHistory bool, rawArguments string) (string, error) {
	...
	return renderAgentResult(skill.Name, strings.Join(results, "\n"), s.useChinese)
}
...
func renderToolDescription(matters []FrontMatter) (string, error) {
	return renderSkillList(matters)
}
