...
func TestTool(t *testing.T) {
	...
	baseDesc, _ := renderToolDescriptionBase(false)
	desc := strings.TrimPrefix(info.Desc, baseDesc)
	assert.Equal(t, `
<available_skills>
<skill>
<name>
...
content1`, result)
	...
	assert.NoError(t, err)
	assert.Equal(t, "skill", info.Name)
	baseDescZh, _ := renderToolDescriptionBase(true)
	desc = strings.TrimPrefix(info.Desc, baseDescZh)
	assert.Equal(t, `
<available_skills>
<skill>
<name>
